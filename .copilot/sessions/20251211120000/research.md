...
- **Async/Await Issues**:
  ...
  - Code analysis of `__init__.py`, `coordinator.py`, and `config_flow.py` shows that `await` *is* being used, which contradicts the warning if the library is indeed async. This points to a potential library-internal issue or a complex race condition.
- **Missing Devices**:
