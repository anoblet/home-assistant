...
- **Fix**:
  ...
  2.  Check if `custom_components/vesync` is filtering them out _before_ logging (unlikely as we checked `coordinator.py`).
  3.  The `is_fan` check in `common.py` might be an issue _if_ they were found, but they aren't even in the logs.
