...
- Room-specific Configuration views in the Rooms dashboard should live at `includes/lovelace/views/<area>/configuration.yaml` with `subview: true`, be included from `includes/lovelace/dashboards/dashboard_room.yaml`, and be opened from the area primary controls with `navigation_path: /dashboard-room/<area>-configuration`.
